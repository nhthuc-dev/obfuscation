...
Gluon (tuỳ chọn): Gluon Client plugin để hỗ trợ JavaFX trên các nền tảng.
GraalVM (tuỳ chọn): GraalVM Community Edition 22.3+ (hoặc mới hơn) để tạo native image.
...
Build và mã hóa với ProGuard:
mvn clean package

File JAR đã mã hóa sẽ nằm tại: target/obfuscated.jar.

Tạo runtime image với jlink:
...
Đóng gói ứng dụng với jpackage:
...
Thêm Gluon Client plugin vào pom.xml:
...
Đóng gói native image với Gluon:
mvn client:package
...
Cài đặt GraalVM và thiết lập JAVA_HOME:
...
Thêm plugin native-maven-plugin vào pom.xml:
...
Thêm file cấu hình GraalVM (src/main/resources/META-INF/native-image/reflect-config.json):
...
Biên dịch Native Image:mvn -Pnative package

File thực thi sẽ nằm tại: target/my-javafx-app.exe.

Cấu hình Gradle cho GraalVM

Thêm plugin org.graalvm.buildtools.native vào build.gradle:
...
Biên dịch Native Image:gradle nativeCompile
...
Chạy từ JAR:java -jar target/obfuscated.jar
...
Chạy từ Native Image (Windows):target/my-javafx-app.exe
